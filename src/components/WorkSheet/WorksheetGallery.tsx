    ...
    const fetchWorksheets = async () => {
      try {
        ...
        if (isFirstRender.current) {
          ...
          // Load saved favorites and ratings from localStorage if available
            ...
            {worksheets.map((worksheet) => (
              <Grid item xs={12} sm={6} md={4} key={worksheet.id}>
                <WorksheetCard
                        ...
                        isLoaded={loadedImages[worksheet.id]} toggleFavorite={function (id: number): void {
                            throw new Error('Function not implemented.');
                        } } handleRatingChange={function (id: number, newValue: number | null): void {
                            throw new Error('Function not implemented.');
                        } }                />
              </Grid>
            ))}
